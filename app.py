...
def fetch_github_file_sha():
  ...
  url = (
      f'https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/contents/{DATA_FILE}'
  )
  headers = {
...
def load_data():
  # 在預設資料中加入 admin_password 欄位，預設為 "admin123"
  default_data = {
      'contact': {
          'region': '雲、嘉、南到府服務',
          'phone': '0988-562-288',
...
def index():
  ...
  for item_id_str, quantity in cart.items():
    ...
    product = next(
        (p for p in data.get('shop_items', []) if p['id'] == item_id), None
    )
    if product:
      try:
        price_val = int(
  ...
  all_user_orders = (
      [o for o in data.get('orders', []) if o.get('username') == logged_in_user]
      if is_logged_in
      else []
  )
  user_orders = [
      ...
      if o.get('status', 'active') in ['active', 'processing', 'shipped']
  ]
  user_history_orders = [
      o for o in all_user_orders if o.get('status') == 'completed'
  ]
  ...
  return render_template(
      ...
      user_history_orders=user_history_orders,
  )
...
def user_complete_order(order_id):
  ...
  for order in data.get('orders', []):
    if (
        ...
        and order.get('username') == logged_in_user
    ):
...
# 修改：後台登入檢查，由原本死板的 'admin123' 改為讀取 json 設定
@app.route('/login', methods=['GET', 'POST'])
def login():
  if request.method == 'POST':
    data = load_data()
    input_password = request.form.get('password', '').strip()
    # 從資料中取得管理員密碼（若不存在則預設為 admin123）
    current_admin_password = data.get('admin_password', 'admin123')

    if input_password == current_admin_password:
...
def handle_message(event):
  ...
  if any(keyword in user_text for keyword in ['真人', '老闆', '人工', '電話', '專人']):
    reply_text = (
        '📞'
        ' 您好！您可以直接撥打建安工作室服務專線：0988-562-288，將由專人為您服務！或者您也可以輸入「找客服」線上留資料。'
    )

  # 2. 觸發機器人登記流程
  elif any(
      keyword in user_text
      for keyword in ['你好我要找客服', '我要找客服', '找客服', '有客服嗎']
  ):
    reply_text = (
        '您好！我是建安工作室的小秘書客服，在這裡為您服務。\n（💡'
  ...
  # 3. 登記步驟：姓名
    ...
    if len(user_text) < 2 or first_char not in common_surnames:
      reply_text = (
          '⚠️'
          ' 感覺名字格式不太對喔！請輸入您的真實中文姓名（例如：陳大明），若想退出請輸入「取消」：'
      )
    else:
      user_state['name'] = user_text
      reply_text = (
          f'收到，您的姓名是【{user_text}】。\n接下來，請輸入您的【連絡電話】（需為 09'
          ' 開頭的 10 碼數字）：'
      )
  ...
  # 4. 登記步驟：電話
    ...
    if len(cleaned_phone) != 10 or not cleaned_phone.startswith('09'):
      reply_text = (
          '⚠️'
          ' 電話格式有誤！請輸入正確的手機號碼（例如：0912345678），若想退出請輸入「取消」：'
      )
    else:
      user_state['phone'] = cleaned_phone
      reply_text = (
          '太好了！最後，請輸入您的【收件/服務地址】（請包含鄉鎮市區與路名）：'
      )
  ...
  # 5. 登記步驟：地址並建立訂單
    ...
    if len(user_text) < 5 or any(w in user_text for w in invalid_words):
      reply_text = (
          '⚠️'
          ' 請輸入詳細的【收件/服務地址】（例如：雲林縣虎尾鎮中正路100號），以便我們安排到府服務：'
      )
    else:
  ...
  # 6. 一般閒聊交給 Gemini AI
    ...
    try:
      ...
      busy_messages = [
          (
              '小秘書目前正在忙線中！如果您想預約服務或買東西，可以直接輸入「找客服」來登記聯絡資訊，或者撥打專線'
              ' 0988-562-288，我們會盡快與您聯絡喔！'
          ),
          (
              '哎呀！系統小幫手現在有點塞車忙不過來了。若有急需服務，歡迎直接輸入「找客服」留資料，或撥打專線'
              ' 0988-562-288 找建安老闆喔！'
          ),
          (
              '不好意思，AI'
              ' 正在休息中！需要二手電腦、監視器或濾水器服務的朋友，請直接輸入「找客服」快速登記，我們會手動為您處理！'
          ),
          (
              '小秘書正在全力服務其他客戶中！您可以直接打專線'
              ' 0988-562-288，或輸入「找客服」留下您的聯絡方式，我們看到會立刻回電！'
          ),
          (
              '系統目前忙碌中，暫時無法自動對話。別擔心！直接輸入「找客服」就能直接進入登記流程，專人會盡快為您安排服務喔！'
          ),
          (
              '哎唷威呀！小秘書的咖啡剛剛打翻了，手忙腳亂中～😅'
              ' 如果有急事找建安老闆，直接打 0988-562-288 或輸入「找客服」最快喔！'
          ),
          (
              '報告老闆！找我的人太多，AI 大腦快燒掉了🔥'
              ' 休息一下下先～您可以直接輸入「找客服」留資料，我們馬上派專人去處理！'
          ),
          (
              '哈哈，被您抓到了！小秘書剛剛分心去偷看貓咪吃飯了 🐾'
              ' 您好呀！有需要電腦或濾水器服務嗎？輸入「找客服」快速幫您登記！'
          ),
          (
              '糟糕，電波好像被雲嘉南的美食香氣干擾了～信號微弱中 🍲😋'
              ' 肚子餓歸肚子餓，正事不能忘！有需要服務請輸入「找客服」喔！'
          ),
          (
              '別急別急～小秘書正在跑步幫您找老闆！🏃‍♂️'
              ' 趕快先輸入「找客服」登記您的需求，老闆看到就會光速飛奔處理囉！'
          ),
      ]
